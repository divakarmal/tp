...
1. The rest of the method seems to exhaustively check for the existence of each possible parameter of the `edit` command and store any possible changes in an `EditPatientDescriptor`. Recall that we can verify the contents of `editPatientDesciptor` through the `Variable` tool window.<br>
